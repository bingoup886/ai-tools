...
#### 🎨 界面视觉升级
- **全新 Banner 设计**：
  - 引入宽屏 Banner 组件，使用高质量插画背景。
  - 优化页面顶部布局，移除重复标题，视觉更加大气。
  - 增加 Banner 遮罩层，确保文字清晰可读。

#### 🔍 搜索功能上线
- **实时搜索**：
  - 在 Banner 区域集成美观的半透明搜索框。
  - 支持按工具名称和描述实时过滤。
  - 智能隐藏无匹配结果的分类，提供清爽的搜索体验。
- **交互优化**：
  - 搜索框聚焦时自动展开并加深背景，提供良好的视觉反馈。
  - 搜索框样式采用毛玻璃效果，完美融入 Banner 背景。

#### 🛠️ 结构优化
- **组件重构**：
  - 拆分 `Banner` 组件，解耦 `Header` 组件职责。
  - 优化 `App.jsx` 状态管理，实现搜索状态在组件间的流畅传递。
  - 迁移管理员模式入口至 Banner 标题，保持原有隐蔽激活方式（点击 5 次）。
